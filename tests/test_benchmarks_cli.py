...
def _register_dummy_model(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setitem(
        cli.MODELS, "dummy", cli.ModelConfig(name="dummy", backends=[b1, b2])
    )
...
def test_summary_save_and_plot(
