...
class AgentSetPolars(AgentSetDF, PolarsMixin):
    ...
    def __init__(self, model: mesa_frames.concrete.model.ModelDF) -> None:
        ...
        self._df = pl.LazyFrame(schema={"unique_id": pl.Int64})
    ...
    def add(
        ...
        elif isinstance(agents, (pl.DataFrame, pl.LazyFrame)):
    ...
    def contains(
        ...
        agent_ids = self._df.collect()["unique_id"]
        if isinstance(agents, pl.Series):
            ...
            return pl.Series(agents, dtype=pl.UInt64).is_in(agent_ids)
        else:
    ...
    def set(
        ...
        obj._update_mask(original_index.collect()["unique_id"], unique_id_column)
        return obj
    ...
    def _concatenate_agentsets(
        ...
        if not duplicates_allowed:
            indices_list = [self._df.collect()["unique_id"]] + [
                agentset._df.collect()["unique_id"] for agentset in agentsets
            ]
        ...
        if not isinstance(original_masked_index, type(None)):
            ids_to_remove = original_masked_index.filter(
                original_masked_index.is_in(self._df.collect()["unique_id"]).not_()
            )
    ...
    def _get_bool_mask(
        ...
        def bool_mask_from_series(mask: pl.Series) -> pl.Series:
            if (
                ...
                and len(mask) == len(self._df.collect())
            ):
                return mask
            return self._df.collect()["unique_id"].is_in(mask)
        ...
        if isinstance(mask, pl.Expr):
            ...
            return pl.repeat(True, len(self._df.collect()))
        elif mask == "active":
            return self._mask
    ...
    def _get_masked_df(
        ...
        if (isinstance(mask, pl.Series) and mask.dtype == pl.Boolean) or isinstance(
            ...
            agents_ids = self._df.collect()["unique_id"]
    ...
    def __iter__(self) -> Iterator[dict[str, Any]]:
        return iter(self._df.collect().iter_rows(named=True))

    def __len__(self) -> int:
        return len(self._df.collect())

    def __reversed__(self) -> Iterator:
        return reversed(iter(self._df.collect().iter_rows(named=True)))
    ...
    @df.setter
    def df(self, agents: pl.DataFrame | pl.LazyFrame) -> None:
        if "unique_id" not in (
        ...
        self._df = agents.lazy() if isinstance(agents, pl.DataFrame) else agents
    ...
    def index(self) -> pl.Series:
        return self._df.collect()["unique_id"]
